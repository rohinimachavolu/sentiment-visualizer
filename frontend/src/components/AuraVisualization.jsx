...
const AuraVisualization = ({ emotion, intensity }) => {
  ...
  const draw = (p5) => {
    ...
    // Smooth lerp transitions (0.03 = slower, smoother)
    ...
    // Draw waves with smoothly transitioning parameters
    const numWaves = Math.floor(currentWaveCountRef.current);
    
    for (let i = 0; i < numWaves; i++) {
      let yBase = (p5.height / (numWaves + 1)) * (i + 1);
      
      // For sad emotion, apply downward drift
      if (emotion === 'sad') {
        yBase += i * 12; // Drift downward
      }
      ...
      // Draw wave with 3 layers for depth
      for (let layer = 0; layer < 3; layer++) {
        p5.beginShape();
        p5.noFill();
        p5.stroke(waveHue, waveSat, waveBright, alpha * [0.25, 0.55, 0.85][layer]);
        p5.strokeWeight([12, 7, 4][layer]); // Thick waves
        
        for (let x = 0; x <= p5.width; x += 4) {
          // Multi-layer Perlin noise for smooth organic flow
          const noise1 = p5.noise(
            x * 0.002,
            yBase * 0.01,
            timeRef.current + i * 0.1
          );
          
          const noise2 = p5.noise(
            x * 0.004,
            yBase * 0.005,
            timeRef.current * 1.3 + i * 0.15
          );
          
          const noise3 = p5.noise(
            x * 0.008,
            yBase * 0.003,
            timeRef.current * 0.7 + i * 0.2
          );
          
          // Sine wave with current frequency (smoothly transitioning)
          const sineWave = p5.sin(
            x * currentFrequencyRef.current + 
            timeRef.current + 
            i * 0.3
          ) * currentAmplitudeRef.current;
          
          // Perlin noise influence (smoothly transitioning turbulence)
    ...
    // Emotion indicator above button
